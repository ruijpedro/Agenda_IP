...
const tabs = [
  ...
  { id: 'agenda', label: 'Agenda Google', icon: CalendarDays },
  ...
  return (
    <header className="hero">
      ...
      <div className="heroText">
        ...
        <small>{account ? `Google ligado: ${account.email || account.name}` : 'Google/Gmail não ligado'}</small>
      </div>

      <button className="pill white" onClick={account ? onLogout : onLogin}>
...
function Dashboard({ items, setItems, connected, settings }) {
  ...
  function save() {
    ...
    saveActivities(next)
    setForm(blank())
    setMsg('Registo guardado.')
  }

  async function exportOne(item) {
    try {
      await createEvent(settings, {
        date: item.data,
        start: item.inicio,
        end: item.fim,
        title: `${item.tipo} - ${item.servico || item.destino || item.local}`,
        location: item.local || item.destino,
        notes: `Técnico: ${item.tecnico}
...
Observações: ${item.observacoes}`
      ...
      setMsg('Exportado para Google Calendar.')
    } catch (e) {
      setMsg(e.message)
    }
  }
  ...
  return (
    <section className="card">
      ...
      <div className="gridForm">
        ...
        <label>Técnico
          <input value={form.tecnico} onChange={e => update('tecnico', e.target.value)} />
        </label>

        <label>Serviço
          <input value={form.servico} onChange={e => update('servico', e.target.value)} placeholder="Ex.: Prevenção Oeste / Inspeção / Reunião" />
        </label>

        <label>Início
          <input type="time" value={form.inicio} onChange={e => update('inicio', e.target.value)} />
        </label>

        <label>Fim
          <input type="time" value={form.fim} onChange={e => update('fim', e.target.value)} />
        </label>

        <label>Matrícula
          <input value={form.matricula} onChange={e => update('matricula', e.target.value.toUpperCase())} placeholder="AA-00-AA" />
        </label>

        <label>Viatura
          <input value={form.viatura} onChange={e => update('viatura', e.target.value)} placeholder="Marca/modelo ou viatura de serviço" />
        </label>
        ...
        {form.tipo === 'Deslocação' && (
          <>
            ...
            <label>Destino
              <input value={form.destino} onChange={e => update('destino', e.target.value)} />
            </label>

            <label>Km inicial
              <input type="number" value={form.kmInicial} onChange={e => update('kmInicial', e.target.value)} />
            </label>

            <label>Km final
              <input type="number" value={form.kmFinal} onChange={e => update('kmFinal', e.target.value)} />
            </label>
          </>
        )}
        ...
        {form.tipo === 'Prevenção' && (
          <>
            ...
            <label>Ocorrência
              <select value={form.ocorrencia} onChange={e => update('ocorrencia', e.target.value)}>
        ...
        <label className="wide">Observações
          <textarea value={form.observacoes} onChange={e => update('observacoes', e.target.value)} placeholder="Notas, ocorrências, referência do serviço, validação..." />
        </label>
      </div>

      <button onClick={save}><Plus /> Guardar</button>
      {msg && <p className="hint">{msg}</p>}

      <h3>Últimos registos</h3>
      <RegisterList items={items.slice(0, 8)} connected={connected} exportOne={exportOne} setItems={setItems} />
    </section>
  )
}
  ...
  return (
    <section className="card">
      <div className="sectionTitle">
        <h2>{title}</h2>
        <div className="search"><Search /><input placeholder="Pesquisar" value={q} onChange={e => setQ(e.target.value)} /></div>
      </div>

      <div className="stats">
        <span>{data.length} registos</span>
        <span>{sumHours(data).toFixed(1)} h</span>
        <span>{sumKm(data)} km</span>
      </div>
...
function Agenda({ settings, connected }) {
  ...
  return (
    <section className="card">
      <div className="sectionTitle">
        <h2>Agenda Google</h2>
        <button disabled={!connected} onClick={load}><RefreshCw /> Atualizar</button>
      </div>

      {!connected && <p className="hint">Opcional: liga Google para exportar/consultar eventos.</p>}
      ...
      <div className="list">
        {events.map(ev => (
          <div className="item" key={ev.id}>
            <b>{ev.summary}</b>
            <span>{new Date(ev.start.dateTime || ev.start.date).toLocaleString('pt-PT')}</span>
  ...
  return (
    <section className="card report">
      <div className="sectionTitle">
        ...
        <button onClick={printPdf}><FileText /> Gerar PDF</button>
      </div>
      ...
      <div className="printArea">
        ...
        <div className="stats">
          ...
          <span>Deslocações: {desloc.length}</span>
          <span>Prevenções: {prev.length}</span>
          <span>Km: {sumKm(desloc)}</span>
          <span>Horas: {sumHours(data).toFixed(1)}</span>
        </div>
          ...
          <tbody>
            {data.map(i => (
              <tr key={i.id}>
                ...
                <td>{i.origem || i.local}</td>
                <td>{i.destino}</td>
                <td>{i.matricula}</td>
                <td>{i.tipo === 'Deslocação' ? Math.max(0, (Number(i.kmFinal) || 0) - (Number(i.kmInicial) || 0)) : ''}</td>
                <td>{i.observacoes}</td>
              </tr>
            ))}
          </tbody>
    ...
    <section className="card">
      ...
      <div className="gridForm">
        <label>Google Client ID
          <input value={local.googleClientId} onChange={e => setLocal({ ...local, googleClientId: e.target.value })} />
        </label>

        <label>Google API Key
          <input value={local.googleApiKey} onChange={e => setLocal({ ...local, googleApiKey: e.target.value })} />
        </label>

        <label className="wide">Apps Script URL
          <input value={local.appsScriptUrl} onChange={e => setLocal({ ...local, appsScriptUrl: e.target.value })} />
        </label>
        ...
        <label>Organização
          <input value={local.organization} onChange={e => setLocal({ ...local, organization: e.target.value })} />
        </label>
      </div>
      ...
      <div className="aboutBox">
        <h2>IP_RJP</h2>
        <p><b>Autor</b><br />Rui Jorge Pedro</p>
        <p>Infraestruturas de Portugal</p>
        <p>© 2026</p>
      </div>
    </section>
  )
...
export default function App() {
  ...
  useEffect(() => {
    getAccount(settings).then(setAccount).catch(() => {})
  }, [settings.googleClientId])

  async function onLogin() {
    try {
      setAccount(await signIn(settings))
    } catch (e) {
      alert(e.message)
    }
  }
