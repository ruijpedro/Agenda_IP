...
function Header({ account, onLogin, onLogout }) {
  return <header className="hero">
    <img src="/logo.png" alt="IP Agenda" />
    <div className="heroText">
      <h1>IP Agenda</h1>
      <p>Registo profissional de deslocações, prevenções e viaturas</p>
      <small>{account ? `Google ligado: ${account.email || account.name}` : 'Modo local ativo · Google/Gmail opcional'}</small>
    </div>
    <button className="pill white" onClick={account ? onLogout : onLogin}>{account ? <LogOut/> : <LogIn/>}{account ? 'Sair' : 'Ligar Google'}</button>
  </header>
}
...
function RegisterList({ items, connected, exportOne, setItems }) {
  ...
  return <div className="list">{items.map(item=><div className="item" key={item.id}>
    ...
    <div className="rowBtns"><button onClick={()=>exportOne?.(item)} disabled={!connected}><CloudUpload/> Google</button><button className="danger" onClick={()=>del(item.id)}><Trash2/> Apagar</button></div>
  </div>)}</div>
}
...
function Agenda({ settings, connected }) {
  ...
  return <section className="card"><div className="sectionTitle"><h2>Agenda Google</h2><button disabled={!connected} onClick={load}><RefreshCw/> Atualizar</button></div>{!connected && <p className="hint">Opcional: liga Google para exportar/consultar eventos.</p>}{error && <p className="error">{error}</p>}<div className="list">{events.map(ev=><div className="item" key={ev.id}><b>{ev.summary}</b><span>{new Date(ev.start.dateTime||ev.start.date).toLocaleString('pt-PT')}</span><small>{ev.location}</small></div>)}</div></section>
}
...
function Report({ items, settings }){
  ...
  return <section className="card report"><div className="sectionTitle"><h2>Relatório mensal</h2><button onClick={printPdf}><FileText/> Gerar PDF</button></div>
    ...
    <div className="printArea">
      <h1>IP Agenda — Deslocações e Prevenções</h1><p><b>Mês:</b> {month} · <b>Autor:</b> {settings.userName} · <b>Organização:</b> {settings.organization}</p>
...
function Definicoes({ settings, setSettings }) {
  ...
  return <section className="card"><h2>Definições</h2><div className="gridForm">
    ...
    <label>Google API Key<input value={local.googleApiKey} onChange={e=>setLocal({...local,googleApiKey:e.target.value})}/></label>
