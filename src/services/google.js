...
export async function sendGmail(settings, to, subject, bodyText) {
  ...
  return googleApi('/gmail/v1/users/me/messages/send', { settings, method: 'POST', body: JSON.stringify({ raw }) })
}
...
export async function callAppsScript(settings, payload) {
  ...
  const res = await fetch(settings.appsScriptUrl.trim(), { method: 'POST', body: JSON.stringify(payload) })
